...
export function MessageList({ messages, isLoading }: MessageListProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto px-4 py-6 justify-center items-center">
                    ...
                    <>
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          default:
                            // Handle tool invocations (type starts with "tool-" or "dynamic-tool")
                            if (part.type.startsWith("tool-") || part.type === "dynamic-tool") {
                              const toolPart = part as any;
                              const toolName = part.type === "dynamic-tool" ? toolPart.toolName : part.type.substring(5);
                              return (
                                <ToolInvocationBadge
                                  key={partIndex}
                                  toolName={toolName}
